Public Partial Class AnnexC
    ...
    Private Sub Headers()
        ...
        Else
            With Response
                ...
                .AddHeader("Content-Disposition", "attachment; filename=""annexc" & sRBSE & ".doc""")
            End With
        End If
    ...
    Private Function LoadCaseDetails() As Boolean
        Try
            ...
            If Not (objCase.GetMinuteDetails(sRBSE, "AnnexC", dtData)) Then
                Throw New Exception("Case.GetMinuteDetails returned False")
            End If

            If dtData.Rows.Count <> 0 Then
                With dtData.Rows(0)
                    lblDate.Text = FormatDate(.Item("AnnexCDate").ToString())

Partial Public Class ActiveMemoFallenStock
    ...
    Private Function LoadCaseDetails() As Boolean
        Try
            ...
            Dim strCPHH As String
            Dim strUnknown As String = "unknown"
            Dim strSamplingDate As String
            Dim strTestResult As String
            If Not (objCase.GetMinuteDetails(sRBSE, "AMFS", dtData)) Then
                Throw New Exception("Case.GetMinuteDetails returned False")
            End If

            If dtData.Rows.Count <> 0 Then
                With dtData.Rows(0)
                    lblRBSE.Text = "RBSE Ref: " & FormatRBSE(.Item("RBSE").ToString())
                    ' lblAHONumber.Text = "AHO: " & .Item("AHONumber").ToString()
                    strCPHH = FormatCPHH(.Item("CPHH").ToString())
                    strCPHH = strCPHH.Substring(0, strCPHH.LastIndexOf("/"))
                    lblCPHH.Text = "CPH: " & strCPHH
                    lblDate.Text = "Date of Report: " & FormatDate(.Item("ActiveMemoDate").ToString())
                    lblFarmAHOName.Text = .Item("FarmAHOName").ToString()
                    lblCaseSurvey.Text = .Item("SurveyDescription").ToString().ToUpper()
                    ...
                    lblTseTestingSiteName.Text = .Item("TseTestingSite").ToString()
                    lblTseTestingSiteAddress.Text = .Item("TSEAddress").ToString()
                    strSamplingDate = .Item("SamplingDate").ToString()
                    If (strSamplingDate = Date.MaxValue.ToString()) Then
                        lblSamplingDate.Text = strUnknown
                    Else
                        lblSamplingDate.Text = FormatDate(strSamplingDate)
                    End If
                    lblApprovalNumber.Text = .Item("TSECPH").ToString()
                    strTestResult = .Item("TestResult").ToString()

                    lblHeadingTestResult.Text = strTestResult.ToUpper()
                    If strTestResult = "Inconclusive" Then
                        lblBodyTestResult.Text = "an " & strTestResult.ToLower()
                    Else
                        lblBodyTestResult.Text = "a " & strTestResult.ToLower()
                    End If

...
public class NewFarmsModel(IAuditLogService auditLogService) : PageModel
{
    ...
    public IReadOnlyList<AuditLogNewFarmEntry> PagedEntries => Entries.Skip((PageNumber - 1) * PageSize).Take(PageSize).ToList();
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        if (Request.Query.ContainsKey(nameof(StartDate)) || Request.Query.ContainsKey(nameof(EndDate)))
        {
            ...
            HasSearched = true;
            Entries = ApplySorting((await auditLogService.GetNewFarmsAsync(StartDate!.Value, EndDate!.Value)).Cast<AuditLogNewFarmEntry>());
        }
        return Page();
    }
    ...
    {
        Func<AuditLogNewFarmEntry, object?> keySelector = SortColumn switch
        {
            "Table" => e => e.TableName,
            "Field" => e => e.FieldName,
            "User" => e => e.UserName,

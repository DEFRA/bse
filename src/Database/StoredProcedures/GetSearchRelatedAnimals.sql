/****** Object:  StoredProcedure [dbo].[GetSearchRelatedAnimals]    Script Date: 18/09/2026 16:36:18 ******/
SET ANSI_NULLS OFF
GO
...
IF OBJECT_ID('[dbo].[GetSearchRelatedAnimals]', 'P') IS NOT NULL
    DROP PROCEDURE [dbo].[GetSearchRelatedAnimals];
GO

CREATE OR ALTER PROCEDURE [dbo].[GetSearchRelatedAnimals]
	@RBSE varchar(9),
	@Name varchar(80),
	@Eartag varchar(48),
	...
	WHERE
		...
		@Name = '' AND
        REPLACE(CASE WHEN [RelatedCase].[RBSE] IS NOT NULL THEN ISNULL([RelatedCase].[EartagCountry], '') + ISNULL([RelatedCase].[EartagHerdmark] + ' ', '') + ISNULL([RelatedCase].[Eartag], '') ELSE ISNULL([CaseRelation].[EartagCountry], '') + ISNULL([CaseRelation].[EartagHerdmark] + ' ', '') + ISNULL([CaseRelation].[Eartag], '') END, ' ', '') LIKE REPLACE(@Eartag, ' ', '') + '%' AND		ISNULL([CaseRelation].[RelationRBSE], '') LIKE @RelationRBSE + '%' AND

...
CREATE PROCEDURE [dbo].[EditCase]
	...
	DECLARE
        ...
        RETURN 0 = success; RETURN 1 = case not found; RETURN 3 = RowStamp mismatch (ConcurrencyViolationException).
        SQL errors propagate via THROW to the caller's CATCH block.
        */
        IF @BirthDate >= '18 July 1988' SET @IsBAB = 1
        ELSE SET @IsBAB = 0

        IF NOT EXISTS (SELECT [RBSE] FROM [Case] WHERE [RBSE] = @RBSE) RETURN 1
	...
	-- Concurrency check: RowStamp mismatch maps to ConcurrencyViolationException in C#

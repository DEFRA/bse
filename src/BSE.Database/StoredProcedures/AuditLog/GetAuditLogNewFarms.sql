

CREATE PROCEDURE GetAuditLogNewFarms
	@StartDate datetime,
	@EndDate datetime AS

	SELECT
		[AuditLog].[ID],
		[AuditLog].[TableName],
		[AuditLog].[FieldName],
		[AuditLog].[LogDate] AS [DateTime],
		[User].[Name] AS [UserName],
		[AuditLog].[BeforeValue],
		[AuditLog].[AfterValue],
		[AuditLog].[Reason],
		ISNULL(CONVERT(varchar(20),LEFT([AuditLog].[RBSE], 2) + '/' + SUBSTRING([AuditLog].[RBSE], 3, 2) + '/' + RIGHT([AuditLog].[RBSE], 5)), CONVERT(varchar(20),LEFT([AuditLog].[CPHH], 2) + '/' + SUBSTRING([AuditLog].[CPHH], 3, 3) + '/' + SUBSTRING([AuditLog].[CPHH], 6, 4) + '/' + RIGHT([AuditLog].[CPHH], 2))) AS [Key],
		[Farm].[OwnerName],
		ISNULL([Farm].[Address1] + ' ', '') + ISNULL([Farm].[Address2] + ' ', '') + ISNULL([Farm].[Address3] + ' ', '') + ISNULL([Farm].[Postcode], '') AS [Address],
		[luBSECounty].[Description] AS [County]
	FROM
		[AuditLog] INNER JOIN [User] ON [AuditLog].[UserID] = [User].[ID]
		LEFT JOIN [Farm] ON [AuditLog].[CPHH] = [Farm].[CPHH]
		LEFT JOIN [luBSECounty] ON [Farm].[County] = [luBSECounty].[Code]
	WHERE
		[Reason] = 'Creation' AND
		[TableName] = 'Farm' AND
		[LogDate] BETWEEN @StartDate AND DATEADD(d, 1, @EndDate)
	ORDER BY
		[LogDate]
